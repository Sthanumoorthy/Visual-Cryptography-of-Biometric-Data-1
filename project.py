...
if (selection == 'q'):
	...
	if selection == 1:
		...
		root=Tk()
		root.withdraw()
		showinfo(title="Please enter a filename:", message=f"Open a file")
		root.update()
		...
		if command == 'r':
			for i in range(file_len):  # Assuming you have 4 shares, adjust if needed
# 				share_filename = input(f"Enter the filename of share {i + 1}: ")
				share_filename=files[i]
				print(share_filename)
				pictures.append(share_filename)
				keyval = int(input(f"Enter key value of share {i + 1}: "))
				key_input = input(f"Enter key value of share {i + 1}: ")  # Add this line
				keyval = int(key_input)  # Now you can use key_input

				try:
						keyval = int(key_input)22
				except ValueError:
					print("Invalid input. Please enter a valid integer for the key value.")
    # You may choose to handle the error or exit the program g
				key.append(keyval)
